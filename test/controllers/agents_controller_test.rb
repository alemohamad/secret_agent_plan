...
class AgentsControllerTest < ActionController::TestCase
  ...
  test 'put update is successful with valid attributes' do
    valid_params = { email: 'updated@email.gov' }
    post :update, id: agents(:bond).id, agent: valid_params
    assert_equal valid_params[:email], agents(:bond).reload.email
    assert_redirected_to agent_path(agents(:bond))
  end

  test 'put update is unsuccessful with invalid attributes' do
    invalid_params = { email: '' }
    post :update, id: agents(:bond).id, agent: invalid_params
    refute_equal invalid_params[:email], agents(:bond).reload.email
